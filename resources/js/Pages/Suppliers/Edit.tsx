...
export default function Edit({ supplier }: { supplier: Supplier }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title={t('suppliers.edit_title')} />

            <PageHeader title={t('suppliers.edit_title')} description={supplier.name}>
                <DangerButton onClick={() => setConfirmDelete(true)}>{t('suppliers.archive_action')}</DangerButton>
            </PageHeader>
            ...
            <GlassCard className="max-w-3xl p-8">
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2">
                        <GlassButton href={route('suppliers.show', supplier.id)} variant="secondary">{t('common.cancel')}</GlassButton>
            ...
            <ConfirmDialog
                ...
                onConfirm={destroy}
                title={t('suppliers.archive_confirm_title')}
                message={t('suppliers.archive_confirm_message', { name: supplier.name })}
