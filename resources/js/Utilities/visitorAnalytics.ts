...
const SESSION_KEY = 'decore.analytics.session';
const SESSION_STARTED_KEY = 'decore.analytics.started';
const HEARTBEAT_INTERVAL = 15_000;

/** Public guest routes worth counting. Admin pages are never tracked. */
const PUBLIC_PREFIXES = ['/', '/catalog', '/about', '/contact', '/gallery'];
...
export function initializeVisitorAnalytics(): void {
    ...
    // Track Inertia navigations between public pages (admin pages are skipped).
    // The `navigate` event fires AFTER the page swap, so window.location already
    // points at the destination page.
